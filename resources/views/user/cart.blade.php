          ...
          <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
              <div class="p-6 bg-white border-b border-gray-200">
                  @if (count($products) > 0)
                    ...
                    <div>
                      <button class="flex ml-auto text-white bg-green-500 border-0 py-2 px-6 focus:outline-none hover:bg-green-600 rounded" onclick="location.href='{{ route('user.cart.success')}}'">在庫を移動する</button>
                    </div>
                  @else
                    カートに商品がありません。
                  @endif
